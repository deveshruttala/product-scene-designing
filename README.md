...
<br>
This project show cases the usage of the models like dreambooth and stable diffusion for the creation of background scene creation and product placement image creation   <br>
...
There are serveral models one can easily make use of to execute image creation. Here i am show casing several models to output our result, 
...
<br>
* Object Transform and Color Augmentations: creates more variation of the object to widen the variety of environments and scenarios it can be placed in.
* Background In-Painting: creates environmental variation in the background.
* Object In-Painting: fills in the background completely for a smoother transition between foreground and background.
<br>
Stable Diffusion In-Painting<br>
...
We can change additional parts of the item with InstructPix2Pix to add even more diversity. InstructPix2Pix is a conditional diffusion technique that combines the capabilities of GPT-3 and stable diffusion to alter photos from human instructions. Since it is currently insufficiently robust to make significant changes to photos without losing semantic information, we will only use the example of changing an object's color.<br>
#####
Above i have shown one implementation with the help of datature nexus implementing using their annotations and the second one is completely a dreambooth implementation using the runwayml/stable-diffusion-v1-5. Another dreambooth implentation can be done by the auto train dreambooth using the model stable- diffusion xl - base 10 (try it out urself). 
<br> 
...
<img src="assets/coke with mountains.png" width="250 " /> <img src="assets/coke with bg.png" width="250" />
</p>
<br>
...
```
 The installation and usage documents are self explainatory  
...
<br>
By the usage of these models there are several applications and image creation that can be done rather than creating a background scene for a product. The scope of the models being vast the image applications can also be boosted for example we can train the model and ask it for outputing images for book or create advertisements etc. <br>

I wanna give a speacial thanks to <a href="https://developers.datature.io/docs/introduction-to-datature">datature</a> and dreambooth model implementors for providing me the access to use their models. 
<br>
In the evolving ai age there are several app that can implement this function like dall e, midjourney, pebbeley, breeze etc , try those softwares out and feel free to pull request me some newer models on this application 
<br>
Happy innovation 
<br>
